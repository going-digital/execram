...
MODIFIED for execram (see src/backends/zopfli_vendor/README.md and
docs/LICENSES.md, per Apache License 2.0 SS4(b)'s "state changes made"
requirement): turned into a no-op. execram's own depacker
(stubs/inflate/, ultimately from Keir Fraser's inflate.S) isn't one of
the buggy decoders this padding exists for, so forcing >=2 distance
codes when a block genuinely needs 0 or 1 only costs bytes here for no
benefit - exactly the tradeoff issue #1 asked to reconsider. Both call
sites below feed the same GetDynamicLengths() and compare patched
output only against other patched output from within this same
function, never against a pre-existing unpatched baseline, so making
this a no-op is behavior-consistent, not just "delete and hope."
*/
