; unzx0_68000_fast_flash.s - flash-instrumented variant of
; unzx0_68000_fast.s (docs/format-spec.md's in-loop decompression
; flicker), vendored for the execram "zx0fast"/"salvadorfast" backends'
; depacker stub.
;
...
; also required by this file's own zlib license ("Altered source
; versions must be plainly marked as such"), see below.
;
; The only changes from unzx0_68000_fast.s: one `move.w d0,(a3)` inserted
; into each of `.copy_lits` and `.copy_match` (D0 is that loop's own
; `dbra` counter - live, changing every pass). A3 itself is set up by
; this file's own stub_fast_flash.s wrapper, before A2 is pushed there -
; this file (unlike unzx0_68000.s) never preserves A2 itself ("trashes:
; d0-d2/a2" per its own header comment below), so the header-pointer-
; still-valid window this needs is in the wrapper, not here. A3 is
; confirmed unused anywhere in this file otherwise.
;
; Source: https://git.platon42.de/chrisly42/unzx0_68000 (main branch,
;   audited 2026-09-16 via the Wayback Machine's 2026-05-11 snapshot,
;   since the live server rejected an anonymous git clone - see
;   docs/LICENSES.md and stubs/zx0/README.md). zlib license, same terms
;   as unzx0_68000.s (this directory) - unmodified below except adding
;   the `Depack` entry point wrapper in stub_fast_flash.s (since, per
;   this file's own header, it does not preserve D2/A2 itself the way
;   unzx0_68000.s does) and the flicker instrumentation described above.
;
...
.copy_lits
        ...
        move.w  d0,(a3)         ; flicker: dbra counter, changes every pass
