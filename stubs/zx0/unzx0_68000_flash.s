; unzx0_68000_flash.s - flash-instrumented variant of unzx0_68000.s
; (docs/format-spec.md's in-loop decompression flicker), vendored for
; the execram "zx0" backend's depacker stub.
;
...
; (zx0 vs zx0fast, lz4small/normal/fast) - also required by this file's
; own zlib license ("Altered source versions must be plainly marked as
; such"), see below.
;
; The only changes from unzx0_68000.s: a `btst`/`lea` pair at the very
; top of `Depack:` (A2 is still the header pointer there - this routine's
; own `movem.l a2/d2,-(sp)` hasn't run yet - reads FLAG_KILLTWITCH and
; loads A3, confirmed unused anywhere else in this file, with the target
; hardware address; A2 itself is never touched here, since Depack's own
; contract requires preserving it - runtime.i re-reads HDR_FLAGS/
; HDR_CODE_DATA_SIZE/HDR_BSS_SIZE via A2 again after this routine
; returns), and one `move.w d0,(a3)` inserted into each of `.copy_lits`
; and `.copy_match` (D0 is that loop's own `dbf` counter - live,
; changing every pass, needs no extra register to source from).
;
; Source: https://github.com/emmanuel-marty/unzx0_68000
;   commit c807773edffae8b12155a980d8031ef7701ccfa1 (audited
;   2026-09-11, see docs/LICENSES.md #3 - zlib license, unmodified
;   below except renaming the entry label `zx0_decompress` to `Depack`,
;   matching stubs/common/runtime.i's calling convention exactly (A0 =
;   compressed input, A1 = output, preserves A2 - runtime.i only
;   requires A2-A6/D2-D7 preserved and this preserves more than asked),
;   and the flicker instrumentation described above.
;
...
.copy_lits:    move.b (a0)+,(a1)+   ; copy literal byte
               move.w d0,(a3)       ; flicker: dbf counter, changes every pass
