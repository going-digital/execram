...
; flicker): a `move.w d0,(a5)` inserted into every one of this file's
; 33 identical per-token dispatch trampolines (the
...
; those jump table entries land *inside* a block at fixed byte offsets
; (e.g. `sl_sm0+4`, `sl_sm0+2`) that a poke inside the copy chain itself
; would silently shift and break - so the poke goes only in the
; dispatch trampoline instead, once per LZ4 *token* rather than once per
; byte (the one backend with different flicker granularity than the
; rest; see the design plan's own note on this trade-off). `a5`/`a6` are
; confirmed unused anywhere else in the original file (grepped
; directly), so this is safe without disturbing any of its register
; conventions. See stub_fast_flash.s's own header comment for where
; `a5` is set up.
