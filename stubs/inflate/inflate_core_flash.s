; inflate_core_flash.s - flash-instrumented variant of inflate_core.s
; (docs/format-spec.md's in-loop decompression flicker) for the execram
; "inflate" backend's depacker stub.
;
...
;
; The only changes from inflate_core.s: a `btst`/`lea` pair inserted
; right after the second `bsr.w build_code` call and before
; `decode_loop:` starts (not any earlier - `build_code` itself clobbers
; A3, confirmed by direct inspection, so the target-address register
; can't be set up before both `build_code` calls finish) - reads the
; FLAG_KILLTWITCH bit cached into D7 by this file's own stub_flash.s
; wrapper (D7 is otherwise untouched anywhere in this file except inside
; `build_code`'s own self-contained `movem.l d0-d7,.../movem.l
; ...,d0-d7` save/restore, confirmed by direct inspection - so a value
; placed there before `inflate:` is first called survives unchanged all
; the way to this insertion point) and loads A3 with the target hardware
; address - and one `move.w d0,(a3)` inserted right after `decode_loop`'s
; own `move.b d0,(a4)+` (D0 is the just-decoded literal byte there, the
; exact value being written to output).
;
...
dh9:   move.l  d0,(a0)+
        ...
        ; docs/format-spec.md's in-loop decompression flicker: A3 can
        ; only be set up here, after both build_code calls finish (it
        ; clobbers A3 itself) and before decode_loop starts (nothing
        ; between here and there touches D7 or A3). D7 = cached
        ; HDR_FLAGS byte, set by stub_flash.s before A2 was overwritten
...
dl1:   bsr.w stream_next_symbol 
        ...
        move.b  d0,(a4)+
        move.w  d0,(a3)          ; flicker: decoded literal byte just written
        bra.s     dl1
